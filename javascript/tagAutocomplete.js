...
async function insertTextAtCursor(textArea, result, tagword, tabCompletedWithoutChoice = false) {
    ...
    // Uses a built-in method from the startfk's ui.js which also already accounts for event target
    tacSelfTrigger = true;
...
async function setup() {
    ...
    applySettingsButton?.addEventListener("click", () => {
        // Wait 500ms to make sure the settings have been applied to the startfk opts object
        setTimeout(async () => { 
            await syncOptions();
        }, 500);
    });
